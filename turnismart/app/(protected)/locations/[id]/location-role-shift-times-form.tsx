...
import { updateLocationRoleShiftTimes } from "@/app/actions/locations";
import type {
  LocationRoleShiftTimesMap,
  LocationRoleShiftTimesWithDays,
} from "@/lib/locations";
import type { RoleShiftTimesMap } from "@/lib/roles";
import { LOCATION_DAY_ALL } from "@/drizzle/schema";
...
export function LocationRoleShiftTimesForm({
  locationId,
  roles,
  roleTimes,
  locationTimes,
  ...
  locationId: string;
  roles: Role[];
  roleTimes: RoleShiftTimesMap;
  locationTimes: LocationRoleShiftTimesWithDays;
}) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const def = locationTimes.default ?? {};
  const byDay = locationTimes.byDay ?? {};
  const defaultTimes = {
  ...
  const [values, setValues] = useState<RoleShiftTimesMap>(() => {
    ...
    for (const r of roles) {
      v[r.id] = def[r.id] ?? roleTimes[r.id] ?? defaultTimes;
    }
    return v;
  });

  const [dayValues, setDayValues] = useState<
    Partial<Record<number, LocationRoleShiftTimesMap>>
  >(() => {
    const v: Partial<Record<number, LocationRoleShiftTimesMap>> = {};
    for (const d of [5, 6] as const) {
      if (byDay[d]) {
        v[d] = {};
        for (const r of roles) {
          v[d]![r.id] =
            byDay[d]![r.id] ?? def[r.id] ?? roleTimes[r.id] ?? defaultTimes;
        }
      }
    }
    return v;
  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        ...
        await updateLocationRoleShiftTimes(locationId, updates);
            ...
            onChange={(e) =>
              setV((prev) => ({
                ...
                [roleId]: {
                  ...prev[roleId],
                  morning: {
                    ...(prev[roleId]?.morning ?? defaultTimes.morning),
            ...
            onChange={(e) =>
              setV((prev) => ({
                ...
                [roleId]: {
                  ...prev[roleId],
                  evening: {
                    ...(prev[roleId]?.evening ?? defaultTimes.evening),
                    start: e.target.value,
                  },
                },
              }))
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900/50">
      <h2 className="font-semibold text-zinc-900 dark:text-white">
        Orari turni per sede e mansione
      </h2>
      <p className="mt-1 text-sm text-zinc-500">
        Override degli orari per questa sede. Sabato e domenica possono avere
        orari diversi.
      </p>
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[400px] text-sm">
          ...
          <tbody>
            {roles.map((r) =>
              renderRow(r.id, r.name, values, setValues)
            )}
          </tbody>
        </table>
      </div>
          ...
          <div className="mt-3 space-y-4">
            {([5, 6] as const).map((day) => {
              const dayMap =
                dayValues[day] ??
                Object.fromEntries(
                  roles.map((x) => [x.id, { ...(values[x.id] ?? defaultTimes) }])
                );
              return (
                <div
                  key={day}
